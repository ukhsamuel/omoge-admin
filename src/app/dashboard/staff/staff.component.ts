...
export class StaffComponent implements OnInit {
  ...
  staff: any;
  temp: any;
  @ViewChild(StaffComponent) table: StaffComponent;
  rows = [];
  ...
  revoke(row) {
    const modalRef = this.modalService.open(RevokeStaffModalContent);
  ...
  getStaff(){
    ...
    this.rest.getStaff().subscribe(response => {
        this._loadingBar.complete();
        this.staff = response.json().data;
...
export class RevokeStaffModalContent {
  @Input() name;
  dataForm: FormGroup;
  loading = false;
  submitted = false;
